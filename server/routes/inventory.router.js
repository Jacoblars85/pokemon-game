...
router.get("/tms", (req, res) => {
  ...
  pool
    ...
    .catch((err) => {
      console.log("ERROR: Get all tms items", err);
      res.sendStatus(500);
    });
});
...
router.get("/user/tms", (req, res) => {
  const query = `
    ...
    FROM "user_inventory"
        INNER JOIN "items"
    ON "user_inventory"."items_id" = "items"."id"
        WHERE "user_id" = $1 AND "user_inventory"."number" > 0 AND "items"."item_type" = 'tms'
        ORDER BY "items_id" ASC;
  `;
  ...
  pool
    ...
    .catch((err) => {
      console.log("ERROR: Get all users tms", err);
      res.sendStatus(500);
    });
});
